...
#define CAMEO_MDIO_DELAY_IN_MS      0
/*tested 90ms is good!*/
#define MDIO_0_1                      0x0
...
void ForDelay()
{    
    if (CAMEO_MDIO_DELAY_IN_MS!=0)
    {
        usleep(CAMEO_MDIO_DELAY_IN_MS);
    }
}
...
int lscpcie_open()
{
	...
	p0 = mmap(0,             /* choose any user address */
		    BarSize,           /* This big */
	...
	Board_fd = fd;
    DEBUG_PRINT("\nPCI_DEV_MEM_SIZE is %d Delay is %d ms \n",PCI_DEV_MEM_SIZE,CAMEO_MDIO_DELAY_IN_MS);
...
typedef int lncd_attr_t[MAX_LANE_NUM][MAX_PARAM_NUM];



lncd_attr_t *pltfm_linecard_serdes_param_query(
    char *vendor, char *speed, char *len, char *type,
    uint8_t card_id, uint8_t chip_id, uint8_t slice_id)
{
    int                 l = 0, m = 0;
    printf("1");
    lncd_attr_t *ptr=NULL;
    printf("2");
    lncd_attr_t obj = {{10, 20}, {30, 40}};
    printf("3");
    ...
    printf("len %s\n",len);
    printf("type %s\n",type);
    for (l = 0; l < MAX_LANE_NUM; l++)
        for (m = 0; m < MAX_PARAM_NUM; m++)
            printf("card(%d)-chip(%d)-slice(%d)-lane(%d)-tap(%d): %d\n", card_id, chip_id, slice_id, l, m+1, obj[l][m]);    
    printf("5");
    printf("*(ptr + 0) = %d\n", (**ptr[0]));
    printf("c ptr = %d \n ",ptr);
    return ptr;   
